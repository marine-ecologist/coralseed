...
seed_settle_particles <- function(
    input, seascape = NULL,
    simulate.mortality = "none", simulate.mortality.n = 0.1,
    competency.function = "exp", limit.time = NA, probability = "additive",
  ...
  if (input == "PalfreyN_PointSource_ForeReefEx_01") {
    load_particles <- PalfreyN_PointSource_ForeReefEx_01
  } else if (input == "PalfreyN_Curtain_ForeReefEx_01") {
    load_particles <- PalfreyN_Curtain_ForeReefEx_01
  } else if (input == "Mermaid_PointSource_Bay_01") {
    load_particles <- Mermaid_PointSource_Bay_01
  } else if (input == "Mermaid_Curtain_Bay_01") {
    load_particles <- Mermaid_Curtain_Bay_01
  } else if (input == "ClamGarden_PointSource_OpenLagoon_01") {
    load_particles <- ClamGarden_PointSource_OpenLagoon_01
  } else if (input == "ClamGarden_Curtain_OpenLagoon_01") {
    load_particles <- ClamGarden_Curtain_OpenLagoon_01
  } else if (input == "SpHub_PointSource_SELaggon_01") {
    load_particles <- SpHub_PointSource_SELaggon_01
  } else if (input == "SpHub_Curtain_SELaggon_01") {
    load_particles <- SpHub_Curtain_SELaggon_01
  } else if (input == "WatsonN_PointSource_ForeReefSh_01") {
    load_particles <- WatsonN_PointSource_ForeReefSh_01
  } else if (input == "WatsonN_Curtain_ForeReefSh_01") {
    load_particles <- WatsonN_Curtain_ForeReefSh_01
  } else {
    load_particles <- sf::st_read(input, drivers = "GeoJSON", quiet = TRUE) |>
      sf::st_zm(drop = TRUE, what = "ZM") |>
      sf::st_transform(20353) |>
      dplyr::select(-decay_value) |>
      dplyr::mutate(time = time + lubridate::hours(14)) # correct time offset for sf::st_read
  }
  ...
  competency_times_output <- predict_competency(particles = n_id, competency.function = competency.function, set.seed = set.seed, return.plot = return.plot)
  ...
  if (silent == FALSE) {
    (cat(paste0("  \n")))
    (cat(paste0(nrow(settled_particles_dispersaltime_df), " / ", length(levels(as.factor(particle_points_probability$id))), " larvae settled \n")))
    (cat(paste0("  \n")))
  }
