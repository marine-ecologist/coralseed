...
// Particle tracks
viewer.dataSources.add(
  Cesium.GeoJsonDataSource.load("geojson/Llines.geojson", {
    clampToGround: true
  }).then(dataSource => {
    const entities = dataSource.entities.values;

    const valid = entities.map(e => {
      const t = e.properties?.time?.getValue();
      const d = t ? new Date(t) : null;
      return d ? { entity: e, date: d } : null;
    }).filter(Boolean);

    const minTime = new Date(Math.min(...valid.map(v => v.date.getTime())));
    const maxTime = new Date(Math.max(...valid.map(v => v.date.getTime())));
    const simStart = Cesium.JulianDate.fromDate(minTime);
    const simEnd = Cesium.JulianDate.fromDate(new Date(maxTime.getTime() + 3600000));
