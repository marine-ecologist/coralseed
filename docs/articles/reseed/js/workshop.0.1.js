...
viewer.camera.setView({
  destination: Cesium.Cartesian3.fromDegrees(146.2, -16.94, 12000),
  orientation: {
...
// 1. Seascape
Cesium.GeoJsonDataSource.load("geojson/moore_seascape.geojson").then(seascapes => {
  ...
  seascapes.entities.values.forEach(entity => {
    ...
    const color = seascapeColors[className] || Cesium.Color.LIGHTGRAY.withAlpha(0.2);
    if (entity.polygon) {
  ...
  // 2. Larval tracks
  return Cesium.GeoJsonDataSource.load("geojson/moore_particle_tracks1.geojson", { clampToGround: true }).then(paths => {
  ...
  // 3. Settlers
  return Cesium.GeoJsonDataSource.load("geojson/moore_settlement_points.geojson", { clampToGround: true }).then(settlers => {
